...
const dropDownData = [
  {
    header: "User Type",
    options: ["Interviewer", "Interviewee"],
  },
  {
  ...
  return (
    <div className="dropdown-and-search-container">
      {dropDownData.map((dropDown) => (
        <div key={dropDown.header}>
          <DropDownForSelect dropDownObj={dropDown} key={dropDown.header} />
        </div>
      ))}
...
function DropDownForSelect({ dropDownObj }) {
  return (
    <div className="custom-button">
      <Dropdown as={ButtonGroup}>
        <Button variant="secondary">{dropDownObj.header}</Button>
        <Dropdown.Toggle split variant="secondary" id="dropdown-split-basic" />
        <Dropdown.Menu>
          {dropDownObj.options.map((option) => (
  ...
  return (
    <div className="input-group-container">
      <InputGroup size="lg" className="input-group">
        <Form.Control placeholder="Start typing the topic..." />
